...
// Relations define how tables connect to each other. This enables Drizzles query API
// to automatically join related data when using `with: { relationName: true }`

// users Relations: A user can have many products and mayn comments
...
// `one()` means a single related record, `many()` means mmultiple related records
